        ...
        <div class="w3-container">
            <center>
              <h1 style="font-size:20pt">Transaksi Pembelian</h1>
            </center>
            <br />
            <br />
            <br />
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('transaksi/mengambilTransaksiPembelian')?>",
            "type": "POST"
        },
...
function setujui_transaksi(id, setujui){
  ...
  if(confirm('Apakah anda yakin untuk '+setujui+' ?'))
  {
    ...
    function detail_transaksi($id){
     ...
     $('#detail > tbody').html('');
     $.getJSON("<?php echo site_url('transaksi/detailTransaksiPembelian')?>/"+$id,
        function (data) {
            console.log(data);
            $.each(data, function (key, value) {           
            $('#detail > tbody').append("<tr><td>"+value.nama_produk+"</td><td>"+value.harga+"</td><td>"+value.jumlah_beli+"</td><td>"+value.tanggal_pelunasan+"</td><td>"+value.waktu_pelunasan+"</td></tr>");
        })
    })
